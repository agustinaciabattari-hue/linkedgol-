...
import { getContentValue, getContentBoolean, getModuleOrder } from "@/lib/site-content";
...
export default function Home() {
  ...
  const modules = getModuleOrder(content, "home_modules_config", DEFAULT_MODULES);
  ...
  const renderModule = (id: string) => {
    switch (id) {
      case "featured_profiles":
        return (
          <section key={id} className="py-20 bg-background">
                      ...
                      <Card className="h-full hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group cursor-pointer border-transparent hover:border-primary/20 overflow-hidden">
                        ...
                        <div className="p-5">
                          ...
                          <div className="flex items-center text-sm text-muted-foreground mt-1 mb-4">
                            ...
                            <span className="truncate">{player.nationality} • {player.position}</span>
                          </div>
      ...
      case "oportunidades":
        return (
          <section key={id} className="py-20 bg-slate-50 border-y border-border">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between mb-10 gap-4">
                <div>
      ...
      case "ofertas_linkedgol":
        if (!curatedOffers || curatedOffers.length === 0) return null;
        return (
          <section key={id} className="py-20 bg-gradient-to-br from-indigo-900 via-indigo-800 to-purple-800">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      case "how_it_works":
        return (
          <section key={id} className="py-20 bg-primary text-primary-foreground relative overflow-hidden">
  ...
  return (
    <div className="min-h-screen">
      <SEO
        ...
        path="/"
      />
      {/* Hero Section — always first, not reorderable */}
      ...
      {/* Modules — order and visibility controlled from the admin */}
      {modules.filter(m => m.visible).map(m => renderModule(m.id))}
    </div>
  );
}
